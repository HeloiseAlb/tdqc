...
class QuantumEnv():
    ...
    def apply_gate_sequence(self,initital_state):
        # DO BE FINISHED
        state = initital_state
        jx_gate_list = self.system.jx_gate_list
        hx_gate_list = self.system.hx_gate_list
        hz_gate_list = self.system.hz_gate_list
        for step in range(0,self.n_steps,1):
            if self.system.gate_order == "zx":
                pass #state = np.dot(a,b)
            elif self.system.gate_order == "xz":
                pass
            print(gate)
            #state = 
        #rho = 
